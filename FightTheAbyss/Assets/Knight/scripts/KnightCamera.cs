...
public class KnightCamera : MonoBehaviour {
    ...
    {
        this.transform.position = Vector3.Lerp(this.transform.position, referencePoint.position, Time.deltaTime * 10);
        /*if (resetOriginalPos)
        {
            if (this.transform.position == originalPosition) {
                resetOriginalPos = false;
            }
            else
            {
                originalPosition = getReferenceCameraPosition(Statics.CameraOriginalReferencePosition).position;
                this.transform.position = Vector3.Lerp(this.transform.position, originalPosition, Time.deltaTime * 4);
            }
        }*/
    }
    ...
    public Transform getReferenceCameraPosition(string name)
    {
        return parent.Find(Statics.CameraReferencePositionContainer).Find(name);
    }

    //Lammada desde Movement
